...
class _CommunityVerificationScreenState extends State<CommunityVerificationScreen>
  ...
  Widget _buildMySubmissionsTab() {
    return Consumer<VerificationProvider>(
      builder: (context, provider, _) {
        // Load user submissions on tab view
        WidgetsBinding.instance.addPostFrameCallback((_) {
          provider.loadUserSubmissions();
        });
...
class _VerificationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final scientificName = treasureData['plantName'] ?? '';
    final confidence = (treasureData['confidence'] ?? 0.0) as double;
    final imageBase64 = treasureData['imageBase64'] ?? treasureData['imageUrl'] ?? '';
    final userName = treasureData['userName'] ?? 'Explorer';
    final discoveredAt = (treasureData['discoveredAt'] as Timestamp?)?.toDate();
