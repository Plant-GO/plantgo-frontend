...
class WalletService {
  ...
  Future<bool> _launchPhantom(Uri url) async {
    try {
      ...
      debugPrint(
        'WalletService: Launch returned false - Phantom may not be installed',
      );
  ...
  Future<bool> checkPhantomInstalled() async {
    // On Android, we can't reliably check if an app is installed
    // due to package visibility restrictions. We'll just try to launch it.
    return true; // Optimistic - will fail gracefully if not installed
  }

  /// Sign and send a transaction using Phantom wallet
  Future<String> signAndSendTransaction(String serializedTx) async {
