...
class BlockchainNFTService {
  static final BlockchainNFTService _instance = BlockchainNFTService._internal();
  ...
  Future<List<BlockchainNFT>> getWalletNFTs(String walletAddress) async {
    try {
      ...
      debugPrint('BlockchainNFTService: Found ${tokenAccounts.length} token accounts');
      ...
      for (final account in tokenAccounts) {
        try {
          ...
          debugPrint('BlockchainNFTService: Error processing token account: $e');
        }
      }
  ...
  Future<Map<String, dynamic>?> _getTokenMetadata(String mintAddress) async {
    try {
      ...
      if (metadataPDA == null) {
        debugPrint('BlockchainNFTService: Could not find metadata PDA for $mintAddress');
        return null;
      }
  ...
  Map<String, dynamic>? _parseMetadataAccount(String base64Data) {
    try {
      ...
      // Read name
      final nameLength = bytes[offset] | (bytes[offset + 1] << 8) | 
                         (bytes[offset + 2] << 16) | (bytes[offset + 3] << 24);
      offset += 4;
      final name = String.fromCharCodes(bytes.sublist(offset, offset + nameLength))
          .replaceAll('\x00', '').trim();
      offset += nameLength;
      
      // Read symbol
      final symbolLength = bytes[offset] | (bytes[offset + 1] << 8) |
                           (bytes[offset + 2] << 16) | (bytes[offset + 3] << 24);
      offset += 4;
      final symbol = String.fromCharCodes(bytes.sublist(offset, offset + symbolLength))
          .replaceAll('\x00', '').trim();
      offset += symbolLength;
      
      // Read URI
      final uriLength = bytes[offset] | (bytes[offset + 1] << 8) |
                        (bytes[offset + 2] << 16) | (bytes[offset + 3] << 24);
      offset += 4;
      final uri = String.fromCharCodes(bytes.sublist(offset, offset + uriLength))
          .replaceAll('\x00', '').trim();
      
      debugPrint('BlockchainNFTService: Parsed metadata - Name: $name, Symbol: $symbol, URI: $uri');
